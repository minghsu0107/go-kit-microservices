...
func proxyingMiddleware(ctx context.Context, instances string, logger log.Logger) ServiceMiddleware {
	...
	for _, instance := range instanceList {
		...
		// When everything is normal, the circuit breaker remains in the closed state
		// and all calls pass through to the services
		// When the number of failures exceeds a predetermined threshold the breaker trips,
		// and it goes into the open state
		// After a timeout period, the circuit switches to a half-open state
		// to test if the underlying problem still exists.
